...
export default function QuranHome() {
  ...
  const filtered = surahs?.filter((s) => s.name.toLowerCase().includes(query.toLowerCase()));

  return (
    <div className="px-5 pt-14 pb-4">
      <div className="flex items-center gap-3 mb-6">
        <div className="w-11 h-11 rounded-xl bg-emerald-soft flex items-center justify-center shrink-0">
          <BookOpenText size={20} className="text-emerald-deep" strokeWidth={1.7} />
        </div>
        <div>
          <h1 className="font-display text-xl font-semibold text-ink">The Noble Quran</h1>
          <p className="text-xs text-ink-soft">114 Surahs · 30 Paras</p>
        </div>
      </div>
      ...
      <div className="mt-4">
        {tab === "surah" ? (
          error ? (
            ...
            <QuranLoading label="Loading Surah list..." />
          ) : (
