  ...
  return (
    <div className="min-h-screen bg-cream">
      ...
      <div className="px-6 pb-10 pt-2 text-center">
        ...
        <h1 className="font-display text-xl font-semibold text-ink mb-8">{khatm.dedicatedTo}</h1>

        <Card className="mb-6">
          <p className="font-arabic text-3xl leading-loose text-emerald-deep mb-5">{duaForKhatm.arabic}</p>
          <div className="h-px bg-emerald-deep/10 my-4" />
          <p className="text-sm text-ink-soft italic mb-3">{duaForKhatm.transliteration}</p>
          <p className="text-[15px] text-ink">{duaForKhatm.meaning}</p>
        </Card>

        <div className="space-y-3">
          <Button className="w-full">Ameen</Button>
          <Button variant="outline" className="w-full" icon={Share2}>Share</Button>
        </div>
      </div>
    </div>
  );
